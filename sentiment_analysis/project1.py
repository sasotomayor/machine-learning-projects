...
def average_perceptron(feature_matrix, labels, T):
    ...
    return (sum_theta/(T * len(feature_matrix)), sum_theta_0/(T * len(feature_matrix)))
...
def pegasos_single_step_update(
    ...
    if (np.matmul(theta, feature_vector) + theta_0) * label <= 1:
...
def classify(feature_matrix, theta, theta_0):
    ...
    classification = np.array(list(map(lambda x: 1 if x > 0 else -1, prediction)))
...
def classifier_accuracy(
    ...
    predictions_traindata = classify(train_feature_matrix, theta, theta_0)
    predictions_valdata = classify(val_feature_matrix, theta, theta_0)

    train_accuracy = accuracy(predictions_traindata, train_labels)
    val_accuracy = accuracy(predictions_valdata, val_labels)
...
def bag_of_words(texts):
    ...
    stopwords = pd.read_csv("stopwords.txt", sep=" ", header=None)[0].to_list()
